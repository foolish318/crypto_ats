...
public final class SelfTestMain {
    ...
    private static void testQuoteValidation() {
        ...
        assertThrows(() -> validator.validate(
                Quote.of(2, "BARC.L", 21_246, 1_000, 21_246, 800, 0)), "locked quote should fail");
        assertThrows(() -> validator.validate(
                Quote.of(3, "", 21_240, 1_000, 21_246, 800, 0)), "blank symbol should fail");
    }

    private static void testDecisionEngine() {
        TradingDecisionEngine engine = new TradingDecisionEngine();
        assertEquals(
                TradingSignal.BUY_PRESSURE,
                engine.evaluate(Quote.of(1, "BARC.L", 21_240, 2_000, 21_244, 500, 0)),
                "buy pressure"
        );
        assertEquals(
                TradingSignal.SELL_PRESSURE,
                engine.evaluate(Quote.of(2, "BARC.L", 21_240, 500, 21_244, 2_000, 0)),
                "sell pressure"
        );
        assertEquals(
                TradingSignal.DO_NOT_TRADE,
                engine.evaluate(Quote.of(3, "BARC.L", 21_240, 2_000, 21_260, 500, 0)),
                "wide spread"
        );
    }
    ...
    private static void testSequencedLocalOrderBook() throws Exception {
        ...
        book.loadSnapshot(
                "{\"lastUpdateId\":100,\"bids\":[[\"1.14060\",\"10.0000\"]],"
                        + "\"asks\":[[\"1.14080\",\"12.0000\"]]}"
        );
        BinanceDepthParser parser = new BinanceDepthParser();

        DepthUpdateApplyResult first = book.apply(parser.parseUpdate(
                "{\"e\":\"depthUpdate\",\"E\":1,\"s\":\"XRPUSDT\",\"U\":99,\"u\":101,"
                        + "\"b\":[[\"1.14070\",\"10.0000\"]],\"a\":[[\"1.14090\",\"12.0000\"]]}"
        ));
        ...
        DepthUpdateApplyResult stale = book.apply(parser.parseUpdate(
                "{\"e\":\"depthUpdate\",\"E\":2,\"s\":\"XRPUSDT\",\"U\":98,\"u\":100,\"b\":[],\"a\":[]}"
        ));
        assertEquals(DepthUpdateApplyResult.STALE, stale, "old update should be stale");

        DepthUpdateApplyResult second = book.apply(parser.parseUpdate(
                "{\"e\":\"depthUpdate\",\"E\":3,\"s\":\"XRPUSDT\",\"U\":102,\"u\":102,\"b\":[],\"a\":[]}"
        ));
        assertEquals(DepthUpdateApplyResult.APPLIED, second, "next contiguous update should apply");

        DepthUpdateApplyResult gap = book.apply(parser.parseUpdate(
                "{\"e\":\"depthUpdate\",\"E\":4,\"s\":\"XRPUSDT\",\"U\":104,\"u\":104,\"b\":[],\"a\":[]}"
        ));
        ...
        book.loadSnapshot(
                "{\"lastUpdateId\":200,\"bids\":[[\"1.14100\",\"10.0000\"]],"
                        + "\"asks\":[[\"1.14130\",\"12.0000\"]]}"
        );
        assertEquals(BookQuality.BOOTSTRAPPING, book.quality(), "resync snapshot should reset quality");
        DepthUpdateApplyResult afterResync = book.apply(parser.parseUpdate(
                "{\"e\":\"depthUpdate\",\"E\":5,\"s\":\"XRPUSDT\",\"U\":199,\"u\":201,"
                        + "\"b\":[[\"1.14110\",\"10.0000\"]],\"a\":[[\"1.14140\",\"12.0000\"]]}"
        ));
